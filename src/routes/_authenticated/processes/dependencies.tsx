...
import { DependencyGraph, CriticalPathChain } from '~/components/wpos/visualizations/DependencyGraph';
import { StatusBadge } from '~/components/wpos/StatusBadge';
import { GitBranch, AlertTriangle, Share2, Shield } from 'lucide-react';

export const Route = createFileRoute('/_authenticated/processes/dependencies')({ component: ProcessDependenciesPage });
...
function ProcessDependenciesPage() {
  ...
  const graph = {
    nodes: [
      ...
      { id: 'p2', name: l === 'ar' ? 'التقاط الموقع' : 'GPS Capture', riskLevel: 'low', criticality: 'medium' },
  ...
  return (<div>
    <PageHeader title="Process Dependency Mapping" titleAr="خريطة تبعيات العمليات" description="Visualize process dependencies" descriptionAr="تصور تبعيات العمليات" currentLang={l} />
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <StatsCard title="Processes" titleAr="العمليات" value="6" icon={<GitBranch />} currentLang={l} />
      <StatsCard title="Dependencies" titleAr="التبعيات" value="5" icon={<Share2 />} currentLang={l} />
      <StatsCard title="Critical Paths" titleAr="مسارات حرجة" value="2" icon={<AlertTriangle />} status="critical" currentLang={l} />
      <StatsCard title="Avg Risk" titleAr="متوسط المخاطرة" value="65%" change={5.3} icon={<Shield />} status="warning" currentLang={l} />
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      <Card><CardHeader><CardTitle>{l === 'ar' ? 'المسار الحرج' : 'Critical Path'}</CardTitle></CardHeader>
        ...
        <div className="mt-4 p-3 bg-red-50 rounded-lg border border-red-200 flex items-center gap-2">
          ...
          <span className="text-sm text-red-700">{l === 'ar' ? 'فشل هذه العمليات يؤثر على التسليم' : 'Failure impacts delivery'}</span>
        </div>
      </Card>
    </div>
  </div>);
