  ...
  const processes = [
    {
      ...
      steps: [
        ...
        { step: 2, name: "Account Setup", duration: "4h", tools: ["AD"], errors: ["Duplicates"] },
        {
          step: 3,
          name: "Orientation",
          duration: "1d",
    ...
    },
    {
      ...
      code: "FIN-INV-001",
      owner: "Fahad Al-Otaibi",
      dept: "Finance",
      risk: "high",
      criticality: "critical",
      steps: [
        { step: 1, name: "Invoice Receipt", duration: "1h", tools: ["Email"], errors: ["Lost"] },
        { step: 2, name: "Verification", duration: "2h", tools: ["ERP"], errors: ["Mismatch"] },
      ],
    },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-3">
          {[...processes].reverse().map((p) => (
            <div
              key={p.id}
              className={`p-4 rounded-xl border cursor-pointer ${selected === p.id ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-white hover:border-blue-300"}`}
            ...
            >
              ...
              <div className="flex gap-2">
                <StatusBadge
                  status={p.risk === "critical" ? "red" : p.risk === "high" ? "yellow" : "green"}
                  size="sm"
                  label={p.risk}
                />
                <StatusBadge
                  status={p.criticality === "critical" ? "red" : "yellow"}
                  size="sm"
                  label={p.criticality}
                />
              </div>
