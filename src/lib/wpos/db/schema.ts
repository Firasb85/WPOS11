...
export const companies = pgTable("companies", {
  ...
  isActive: boolean("is_active").default(true),
  tier: varchar("tier", { length: 20 }).notNull().default("professional"),
  pilotExpiresAt: timestamp("pilot_expires_at", { withTimezone: true }),
...
export const employees = pgTable(
  ...
  (t) => ({
    idxTeam: index("idx_emp_team").on(t.teamId),
    idxSupervisor: index("idx_emp_supervisor").on(t.supervisorId),
  }),
);
...
export const performanceSnapshots = pgTable(
  ...
  (t) => ({
    idxEmployee: index("idx_snap_employee").on(t.employeeId),
    idxKpi: index("idx_snap_kpi").on(t.kpiId),
    idxStatus: index("idx_snap_status").on(t.status),
  }),
);
...
export const evidence = pgTable(
  ...
  (t) => ({
    idxEmployee: index("idx_ev_employee").on(t.employeeId),
    idxType: index("idx_ev_type").on(t.evidenceType),
  }),
);
...
export const diagnosticReports = pgTable(
  ...
  (t) => ({
    idxEmployee: index("idx_diag_employee").on(t.employeeId),
    idxStatus: index("idx_diag_status").on(t.status),
  }),
);
...
export const diagnosticHypotheses = pgTable(
  "diagnostic_hypotheses",
  {
    ...
    reportId: uuid("report_id")
      .notNull()
      .references(() => diagnosticReports.id, { onDelete: "cascade" }),
...
export const cases = pgTable(
  ...
  (t) => ({
    idxEmployee: index("idx_case_employee").on(t.employeeId),
    idxStatus: index("idx_case_status").on(t.status),
  }),
);
